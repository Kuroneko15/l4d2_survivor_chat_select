...
void ItemHandler(TopMenu topmenu, TopMenuAction action, TopMenuObject object_id, int param, char[] buffer, int maxlength) {
	switch (action) {
		case TopMenuAction_DisplayOption:
			FormatEx(buffer, maxlength, "Change Survivor Model");
...
bool CanUse(int client, bool checkAdmin = true) {
	if (!client || !IsClientInGame(client)) {
		ReplyToCommand(client, "The character selection menu is only available for players in the game.");
		return false;
	}
	...
	if (GetClientTeam(client) != 2) {
		ReplyToCommand(client, "The character selection menu is only available for survivors.");
		return false;
	}

	if (L4D_IsPlayerStaggering(client)) {
		ReplyToCommand(client, "This command cannot be used in the hard state.");
		return false;
	}

	if (IsGettingUp(client)) {
		ReplyToCommand(client, "This command cannot be used while getting up.");
		return false;
	}
